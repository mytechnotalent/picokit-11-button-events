...
FRAME_CT = "e0f2ae2cf9937eb0c87847f9b6d5bb49b87f5ce278"
FRAME_TAG = "ed66377a8258a44018fac3d17602d22b"
PLAINTEXT = b'{"n":11,"s":12,"e":2}'
...
def test_envelope_matches_firmware():
    ...
    ciphertext, tag = crypto._xchacha_seal(
        crypto.derive_field_key(), nonce, bytes([11]), PLAINTEXT)
...
def test_public_aead_round_trip():
    ...
    nonce = bytes(range(24))
    ciphertext, tag = crypto.xchacha_seal(key, nonce, b"\x0b", PLAINTEXT)
    opened = crypto.xchacha_open(key, nonce, b"\x0b", ciphertext, tag)
