/**
 * FILE: test_picokit_11_button_events_and_security.c
 *
 * DESCRIPTION:
 * Native unit tests for the RP2350 Picokit button events node:
 * provisioning constants, the packet artifact, CRC, the RYLR998 AT
 * interface, the debounced button input, and the button events state
 * machine that classifies short, long, and double presses with an
 * authenticated LoRa heartbeat.
 *
 * BRIEF:
 * Native unit test runner for picokit-11-button-events.
 *
 * AUTHOR: Kevin Thomas
 * DATE: September 2026
 */
...
static void reset_all(void) {
    ...
    mock_uart_reset();
    button_reset();
    gpio_put(PICOKIT_11_BUTTON_EVENTS_BUTTON_PIN, true);
}
...
static void assert_pin_constants(void) {
    TEST_ASSERT_EQUAL_UINT(25u, PICOKIT_11_BUTTON_EVENTS_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(8u, PICOKIT_11_BUTTON_EVENTS_UART_TX);
    TEST_ASSERT_EQUAL_UINT(9u, PICOKIT_11_BUTTON_EVENTS_UART_RX);
    TEST_ASSERT_EQUAL_UINT(16u, PICOKIT_11_BUTTON_EVENTS_RED_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(17u, PICOKIT_11_BUTTON_EVENTS_YELLOW_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(18u, PICOKIT_11_BUTTON_EVENTS_GREEN_LED_PIN);
    TEST_ASSERT_EQUAL_UINT(15u, PICOKIT_11_BUTTON_EVENTS_BUTTON_PIN);
}
...
static void assert_frame_constants(void) {
    TEST_ASSERT_EQUAL_UINT(115200u, PICOKIT_11_BUTTON_EVENTS_UART_BAUD);
    TEST_ASSERT_EQUAL_UINT(48u, PICOKIT_11_BUTTON_EVENTS_FRAME_SIZE);
    TEST_ASSERT_EQUAL_UINT(5000u, PICOKIT_11_BUTTON_EVENTS_TX_INTERVAL_MS);
...
static void assert_artifact_ids(void) {
    ...
    TEST_ASSERT_EQUAL_UINT(11u, PACKET_NODE_ID);
...
static void assert_artifact_example(void) {
    ...
    TEST_ASSERT_EQUAL_UINT8(0x31u, PACKET_EXAMPLE_FRAME[6]);
    TEST_ASSERT_EQUAL_UINT8(0x65u, PACKET_EXAMPLE_FRAME[15]);
...
static void press_button(void) {
    gpio_put(PICOKIT_11_BUTTON_EVENTS_BUTTON_PIN, false);
}
...
void test_monitor_build_frame_json(void) {
    ...
    g_seq = 12u;
    g_event = MONITOR_EVENT_LONG;
    monitor_build_frame(frame, sizeof(frame));
    TEST_ASSERT_EQUAL_STRING("{\"n\":11,\"s\":12,\"e\":2}", frame);
}
...
void test_monitor_init(void) {
    ...
    TEST_ASSERT_TRUE(s_mock_gpio_dirs[PICOKIT_11_BUTTON_EVENTS_LED_PIN]);
    TEST_ASSERT_FALSE(s_mock_gpio_dirs[PICOKIT_11_BUTTON_EVENTS_BUTTON_PIN]);
}

void test_monitor_short(void) {
    init_monitor();
    press_button();
    step_at(mock_timer_now_us());
    release_button();
    step_at(mock_timer_now_us() + 100000u);
    step_at(mock_timer_now_us() + MONITOR_DOUBLE_GAP_US + 100000u);
    TEST_ASSERT_EQUAL_UINT(MONITOR_EVENT_SHORT, (unsigned)g_event);
}

void test_monitor_long(void) {
    init_monitor();
    press_button();
    step_at(mock_timer_now_us());
    step_at(mock_timer_now_us() + MONITOR_LONG_PRESS_US + 100000u);
    release_button();
    step_at(mock_timer_now_us() + 100000u);
    TEST_ASSERT_EQUAL_UINT(MONITOR_EVENT_LONG, (unsigned)g_event);
}

void test_monitor_double(void) {
    init_monitor();
    press_button();
    step_at(mock_timer_now_us());
    release_button();
    step_at(mock_timer_now_us() + 50000u);
    press_button();
    step_at(mock_timer_now_us() + 50000u);
    TEST_ASSERT_EQUAL_UINT(MONITOR_EVENT_DOUBLE, (unsigned)g_event);
}
...
/**
 * @brief Run the button events monitor state-machine tests.
 *
 * @param void No parameters.
 * @return void
 */
static void run_monitor_tests(void) {
    ...
    RUN_TEST(test_monitor_short);
    RUN_TEST(test_monitor_long);
    RUN_TEST(test_monitor_double);
...
/**
 * @brief Run the monitor edge and button input test groups.
 *
 * @param void No parameters.
 * @return void
 */
